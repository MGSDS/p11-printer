"""CLI for P11 thermal label printer."""
...
from p11.imaging import image_to_raster, prepare_image, text_to_image
        ...
        if args.setting == "density":
            ...
            if args.value in types:
                ...
                if not 0 <= val <= 2:
                    print("  ERROR: paper must be gap, black, continuous, or 0-2")
                    return
            await pc.set_paper_type(val)
            print(f"  Set paper={args.value}: OK")


def _add_paper_arg(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--paper", type=str, default="gap",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="P11 Label Printer")
    ...
    p_text.add_argument("--copies", type=int, default=1)
    p_text.add_argument("--font-size", type=int, default=30)
    p_text.add_argument("--label-length", type=int, default=None,
                        help="Label length in mm")
    ...
    p_image.add_argument("--copies", type=int, default=1)
